...
def scrape_seas_nve():
    plans = []
    try:
        url = "https://www.seas-nve.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'plan_navn': f'SEAS-NVE Fiber {speed}',
                        'beskrivelse': f'SEAS-NVE Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Lokalt selskab'],
                        'rating': 4.0
                    })
            except Exception as e:
                logger.warning(f"Error parsing SEAS-NVE plan: {e}")
                continue
        
        logger.info(f"Scraped {len(plans)} plans from SEAS-NVE")
    except Exception as e:
        logger.error(f"Error scraping SEAS-NVE: {e}")
    return plans
