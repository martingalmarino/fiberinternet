...
def scrape_altibox():
    plans = []
    try:
        url = "https://www.altibox.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'plan_navn': f'Altibox Fiber {speed}',
                        'beskrivelse': f'Altibox Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Altibox TV'],
                        'rating': 4.2
                    })
            except Exception as e:
                logger.warning(f"Error parsing Altibox plan: {e}")
                continue
        
        logger.info(f"Scraped {len(plans)} plans from Altibox")
    except Exception as e:
        logger.error(f"Error scraping Altibox: {e}")
    return plans
