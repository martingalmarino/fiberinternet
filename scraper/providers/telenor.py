...
def scrape_telenor():
    """Scrape fiber internet plans from Telenor.dk"""
    plans = []
    
    try:
        url = "https://www.telenor.dk/privat/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'pris_mdr': price,
                        'bindingstid_mdr': 24,
                        'kampagne': '200 kr. rabat første 3 måneder',
                        'plan_navn': f'Telenor Fiber {speed}',
                        'beskrivelse': f'Telenor Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Premium support'],
                        'rating': 4.4
                    })
        ...
        logger.info(f"Scraped {len(plans)} plans from Telenor")
        
    except Exception as e:
        logger.error(f"Error scraping Telenor: {e}")
