...
def scrape_fibia():
    plans = []
    try:
        url = "https://www.fibia.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'plan_navn': f'Fibia Fiber {speed}',
                        'beskrivelse': f'Fibia Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Lokalt selskab'],
                        'rating': 4.1
                    })
            except Exception as e:
                logger.warning(f"Error parsing Fibia plan: {e}")
                continue
        
        logger.info(f"Scraped {len(plans)} plans from Fibia")
    except Exception as e:
        logger.error(f"Error scraping Fibia: {e}")
    return plans
