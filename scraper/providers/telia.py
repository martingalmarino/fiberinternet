...
def scrape_telia():
    """Scrape fiber internet plans from Telia.dk"""
    plans = []
    
    try:
        # Telia fiber internet page
        url = "https://www.telia.dk/privat/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'kampagne': promotion,
                        'plan_navn': plan_name or f'Telia Fiber {speed}',
                        'beskrivelse': f'Telia Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Telia TV inkluderet'],
                        'rating': 4.1
                    })
                    
            except Exception as e:
                logger.warning(f"Error parsing Telia plan section: {e}")
                continue
        ...
        if not plans:
            ...
            for table in tables:
                ...
                for row in rows:
                    ...
                    if len(cells) >= 3:
                        try:
                            # Look for speed and price in cells
                            ...
                            if speed > 0 and price > 0:
                                plans.append({
                                    ...
                                    'bindingstid_mdr': 12,
                                    'kampagne': '',
                                    'plan_navn': f'Telia Fiber {speed}',
                                    'beskrivelse': f'Telia Fiber {speed} Mbit/s',
                                    'features': ['Gratis installation'],
                                    'rating': 4.1
                                })
                        except Exception as e:
                            logger.warning(f"Error parsing Telia table row: {e}")
                            continue
        
        logger.info(f"Scraped {len(plans)} plans from Telia")
        
    except Exception as e:
        logger.error(f"Error scraping Telia: {e}")
