...
def scrape_kviknet():
    plans = []
    try:
        url = "https://www.kviknet.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'pris_mdr': price,
                        'bindingstid_mdr': 12,
                        'kampagne': 'Gratis hastighedstest og installation',
                        'plan_navn': f'Kviknet Fiber {speed}',
                        'beskrivelse': f'Kviknet Fiber {speed} Mbit/s',
                        'features': ['Gratis installation', 'Hastighedsgaranti'],
                        'rating': 4.6
                    })
            except Exception as e:
                logger.warning(f"Error parsing Kviknet plan: {e}")
                continue
        
        logger.info(f"Scraped {len(plans)} plans from Kviknet")
    except Exception as e:
        logger.error(f"Error scraping Kviknet: {e}")
    return plans
