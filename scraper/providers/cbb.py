...
def scrape_cbb():
    plans = []
    try:
        url = "https://www.cbb.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'pris_mdr': price,
                        'bindingstid_mdr': 24,
                        'kampagne': 'Gratis opgradering til 500 Mbit/s',
                        'plan_navn': f'CBB Fiber {speed}',
                        'beskrivelse': f'CBB Fiber {speed} Mbit/s',
        ...
        logger.info(f"Scraped {len(plans)} plans from CBB")
    except Exception as e:
        logger.error(f"Error scraping CBB: {e}")
    return plans
