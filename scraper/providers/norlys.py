...
def scrape_norlys():
    plans = []
    try:
        url = "https://www.norlys.dk/internet/fiber"
            ...
            try:
                ...
                if speed > 0 and price > 0:
                    plans.append({
                        ...
                        'plan_navn': f'Norlys Fiber {speed}',
                        'beskrivelse': f'Norlys Fiber {speed} Mbit/s',
        ...
        logger.info(f"Scraped {len(plans)} plans from Norlys")
    except Exception as e:
        logger.error(f"Error scraping Norlys: {e}")
    return plans
